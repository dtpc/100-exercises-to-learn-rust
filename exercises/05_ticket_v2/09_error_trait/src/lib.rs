...
impl fmt::Display for TicketNewError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err = match self {
            TicketNewError::DescriptionError(e) => e,
            TicketNewError::TitleError(e) => e,
        };
        write!(f, "{}", err) 
    }
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::DescriptionError(_)) => easy_ticket(
            title, "Description not provided".to_string(), status
        ),
        Err(err) => panic!("{err}"),
    }
}
