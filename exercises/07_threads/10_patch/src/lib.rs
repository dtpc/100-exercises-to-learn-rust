...
impl TicketStoreClient {
    ...
    pub fn update(&self, patch: TicketPatch) -> Result<(), OverloadedError> {
        let (response_sender, response_receiver) = sync_channel(1);
        self.sender.try_send(Command::Update { patch, response_channel: response_sender } )
        .map_err(|_| OverloadedError)?;
        Ok(response_receiver.recv().unwrap())
    }
        ...
        match receiver.recv() {
            ...
            Ok(Command::Update {
                ...
                if let Some(ticket) = store.get_mut(patch.id) {
                    optional_update(&mut ticket.title, patch.title);
                    optional_update(&mut ticket.description, patch.description);
                    optional_update(&mut ticket.status, patch.status);
                }
...
fn optional_update<T>(var: &mut T, opt: Option<T>) {
    if let Some(x) = opt { *var = x };
}
